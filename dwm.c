...
resizerequest(XEvent *e)
{
	...
	if ((i = wintosystrayicon(ev->window))) {
		...
		XMoveResizeWindow(dpy, selmon->barwin, selmon->wx + sp, selmon->by + vp, selmon->ww - 2 * sp - getsystraywidth(), bh);
		updatesystray();
	}
}
...
updatesystray(void)
{
	...
	//TEST
	//return;
	//END TEST, works till here, no systray displayed
	w = w ? w + systrayspacing : 1;
	x -= w;
	XMoveResizeWindow(dpy, systray->win, x, m->by, w, bh);
	//TEST
	//return;
	//END TEST, works till here, systray displayed!
	...
	//TEST, still works
	//return;
	//TODO FIXME this part seems to break!
